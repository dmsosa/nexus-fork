// Configuración central del sitio.
// Cambia aquí los enlaces y textos que se repiten en toda la web.

export const siteConfig = {
  nombreComunidad: 'Liga Nexus',

  // Enlace provisional de WhatsApp (grupo o chat de invitación)
  whatsappUrl: 'https://chat.whatsapp.com/TU-ENLACE-AQUI',

  // Formulario / correo para solicitar acceso al repositorio privado (Git)
  // Puedes apuntarlo a un Google Form, Typeform, o a un mailto:
  solicitudGitUrl: 'mailto:tu-correo@dominio.com?subject=Solicitud%20de%20acceso%20al%20repositorio',

  email: 'tu-correo@dominio.com',

  // Redes / canales de la comunidad (edita con tus enlaces reales)
  slackUrl: 'https://join.slack.com/TU-ENLACE-AQUI',
  githubUrl: 'https://github.com/TU-ORGANIZACION',
  linkedinUrl: 'https://www.linkedin.com/company/TU-EMPRESA',
}
